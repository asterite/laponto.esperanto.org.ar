<table border="0" cellpadding="4" cellspacing="1" width="100%">
	<tr>
			...
			<td class="listgridHeader" nowrap>
				...
				if ($header->allowsOrder()) {
					if ($header->isAscendentOrder()) {
						?>
						<img src='/interfaco/images/grid_order_up_active.gif' width='6' height='5'>
						<?
					} else {
						?>
						<a onMouseover="showtip(this,event,'Orden Creciente')"
							...
							src='/interfaco/images/grid_order_up.gif' width='6' height='5' border='0'></a>
						<?
					}
					if ($header->isDescendentOrder()) {
						?>
						<img src='/interfaco/images/grid_order_down_active.gif' width='6' height='5'>
						<?
					} else {
						?>
						<a onMouseover="showtip(this,event,'Orden Decreciente')"
							onMouseout="hidetip()" href="<?= $header->getLinkDescendentOrder() ?>"><img
							src='/interfaco/images/grid_order_down.gif' width='6' height='5' border='0'></a>
						<?
					}
				}
				?>
		...
		if ($list->hasOptions()) {
			?>
			<td class="listgridHeader" nowrap>&#348;an&#285;i</td>
			<?
		}
		?>
	</tr>
			...
			if ($list->hasOptions()) {
				...
				if ($row->hasEditPage()) {
					...
					<a onMouseover="showtip(this,event,'Editar')"
						onMouseout="hidetip()"
						href="<?= $row->getLinkEditPage() ?>"><img src="<?= $context_path ?>/interfaco/images/grid_option_edit.gif"
				...
				if ($row->hasDeletePage()) {
					...
					<a onMouseover="showtip(this,event,'Eliminar')"
						...
						src="<?= $context_path ?>/interfaco/images/grid_option_delete.gif" width="14" height="15" border="0"></a>
					<?
				}
